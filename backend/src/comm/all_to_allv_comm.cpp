...
void all_to_all_comm(vector_buffer* vectorized_send_buffer, int vectorized_send_buffer_size, int* send_counts, int *recv_buffer_size, u64 **recv_buffer, MPI_Comm comm)
{
    ...
    *recv_buffer = new u64[*recv_buffer_size];
    ...
    twophase_rbruck_alltoallv(r, send_buffer, send_counts, send_displacements, MPI_UNSIGNED_LONG_LONG, *recv_buffer, recv_counts, recv_displacements, MPI_UNSIGNED_LONG_LONG, comm);
...
void comm_compaction_all_to_all(all_to_allv_buffer compute_buffer, int **recv_buffer_counts, u64 **recv_buffer, MPI_Comm comm)
{
    ...
    *recv_buffer = new u64[outer_hash_buffer_size];
    ...
    twophase_rbruck_alltoallv(r, send_buffer, compute_buffer.cumulative_tuple_process_map, send_disp, MPI_UNSIGNED_LONG_LONG, *recv_buffer, recv_counts, recv_displacements, MPI_UNSIGNED_LONG_LONG, comm);
		...
		for (int z = ze-1; z > 0; z--) {
			...
			for (int i = 0; i < di; i++) {
				...
				if (pos_status[send_index] == 0)
					memcpy(&temp_send_buffer[offset], &sendcopy[sdispls[send_index]*typesize], sendNcopy[send_index]*typesize);
				else
